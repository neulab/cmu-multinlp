...
def mix_dist(weight, param_name, num_layers=12):
...
class BertEmbedder(TokenEmbedder):
    ...
    def __init__(self,
                 ...
                 tasks: Union[List[str], Dict[str, int]] = None,
                 max_pieces: int = 512,
                 num_start_tokens: int = 1,
                 num_end_tokens: int = 1,
                 method: str = None,
                 use_middle_layer: int = None) -> None:
        super().__init__()
        ...
        self.use_middle_layer = use_middle_layer
    ...
    def forward(self,
        ...
        if not self.top_layer_only:
            if hasattr(self, '_scalar_mix_{}'.format(self._task)):
                scalar_mix = getattr(self, '_scalar_mix_{}'.format(self._task))
                mix = scalar_mix(recombined_embeddings, input_mask)
            else:
                mix = recombined_embeddings[self.task2layer[self._task]]
        else:
            mix = recombined_embeddings[-1]
            if self.use_middle_layer is not None:
                middle = recombined_embeddings[self.use_middle_layer]
        ...
        if offsets is None:
            ...
            final = util.uncombine_initial_dims(mix, dims)
            if self.use_middle_layer is not None:
                return final, util.uncombine_initial_dims(middle, dims)
            return final
        else:
            ...
            final = util.uncombine_initial_dims(selected_embeddings, offsets.size())
            if self.use_middle_layer is not None:
                selected_middle = middle[range_vector, offsets2d]
                return final, util.uncombine_initial_dims(selected_middle, offsets.size())
            return final


@TokenEmbedder.register("bert-pretrained-multitask")
class PretrainedBertEmbedder(BertEmbedder):
    ...
    def __init__(self,
        ...
        super().__init__(bert_model=model, top_layer_only=top_layer_only,
                         tasks=tasks, method=method, use_middle_layer=use_middle_layer)
